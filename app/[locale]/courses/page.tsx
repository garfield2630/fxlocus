    ...
    <div className="space-y-14 md:space-y-20">
      ...
      <section className="fx-section">
        <div className="grid gap-4 md:grid-cols-2">
          {courses.map((course) => (
            <Link key={course.slug} href={`/courses/${course.slug}`} locale={locale} className="fx-card block p-7">
              <div className="flex items-center justify-between gap-3">
                <span className="fx-pill">
                  {tCommon(`labels.${labelKey(course.pillar)}` as any)}
                </span>
                <span className="text-xs text-slate-200/60">
                  {t(`access.${course.access}`)} · {t(`level.${course.level}`)}
                </span>
              </div>
