...
function buildStudent3DOption(locale: "zh" | "en", items: StudentSummary["items"], totalCourses: number) {
  ...
  return {
    ...
    xAxis3D: { type: "category", data: x, name: locale === "zh" ? "课节" : "Lesson" },
    yAxis3D: { type: "category", data: y, name: locale === "zh" ? "状态" : "Status" },
    zAxis3D: { type: "value", name: locale === "zh" ? "进度" : "Progress", min: 0, max: 100 },
    grid3D: {
      boxWidth: 180,
      boxDepth: 80,
      viewControl: { alpha: 30, beta: 20, distance: 220 },
...
function buildAdmin3DOption(locale: "zh" | "en", byStatus: Record<string, number>) {
  ...
  return {
    ...
    xAxis3D: { type: "category", data: x, name: locale === "zh" ? "学员状态" : "Student status" },
    yAxis3D: { type: "category", data: y },
    zAxis3D: { type: "value", min: 0, max },
    grid3D: {
      boxWidth: 160,
      boxDepth: 40,
      viewControl: { alpha: 25, beta: 35, distance: 200 },
      light: { main: { intensity: 1.1 }, ambient: { intensity: 0.35 } }
    },
    series: [{ type: "bar3D", data, shading: "lambert", bevelSize: 0.25 }]
  };
}
                  ...
                  data.latest.map((row) => (
                    <div key={row.course_id} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3">
                      ...
                      <div className="text-white/50 text-xs">
                        {row.updated_at ? new Date(row.updated_at).toLocaleString(locale === "zh" ? "zh-CN" : "en-US") : ""}
                      </div>
